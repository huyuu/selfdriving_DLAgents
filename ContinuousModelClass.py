...
class DeepModel():
    def __init__(self):
        ...
        self.traceDataDirPath = '../data/trace1'
        # load model
        self.modelPath = './continuousModel.h5'
    ...
    def showVedio(self):
        fig = pl.figure()
        plottingImages = []
        images, subParas, labels = self.__loadOneTrainingDataSetByBatch()
        for image in images:
            image = getCenterDeviationWithImage(image, shouldTrim=True)
    ...
    def __loadModel(self):
        ...
        #     return kr.models.load_model('continuousModel_succeed.h5')
            ...
            image_inputs = kr.layers.Input(shape=(60, 320, 1), dtype=np.float, name='image')
            ...
            num_actions = 1
            # common = kr.layers.Dense(128, activation="relu", name='common_dense1')(common)
    ...
    def __loadOneTrainingDataSetByBatch(self):
        dataSetDirs = list(filter(lambda name: '2021' in name, os.listdir(f"{self.traceDataDirPath}/")))
