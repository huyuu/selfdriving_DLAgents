...
class DeepModel():
    ...
    def run(self):
        ...
        results = self.model.evaluate(images_test, labels_test, batch_size=16)
    ...
    def __loadModel(self):
        if os.path.exists(self.modelPath):
            ...
            # action = kr.layers.Dense(num_actions, name='action_dense2')(common)
            action = kr.layers.Dense(num_actions, name='action_dense2')(image_layer)

            # model = kr.Model(inputs=[image_inputs, subPara_inputs], outputs=action)
            model = kr.Model(inputs=image_inputs, outputs=action)
            model.compile(
